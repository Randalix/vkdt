    ...
    if((num % 4) == 0 && num+4 <= count)
    {
      ...
      for(int i = 0; i < 48; i++)
      {
        ...
        nk_fill_arc(cmd, ccx, ccy, radius, a0, a1, nk_rgba_f(
              0.5f + 0.5f * cosf(mid_a),
              0.5f + 0.5f * cosf(mid_a - 2.0f*M_PI/3.0f),
              0.5f + 0.5f * cosf(mid_a + 2.0f*M_PI/3.0f), 1.0f));
      }
      ...
      float wx = disp[0] + disp[1]*cosf(2.0f*M_PI/3.0f) + disp[2]*cosf(4.0f*M_PI/3.0f);
      float wy = disp[1]*sinf(2.0f*M_PI/3.0f) + disp[2]*sinf(4.0f*M_PI/3.0f);
      ...
      { // pure delta dragging
        ...
        disp[0] += 2.0f/3.0f * sx;
        disp[1] += -1.0f/3.0f * sx + 1.0f/sqrtf(3.0f) * sy;
        disp[2] += -1.0f/3.0f * sx - 1.0f/sqrtf(3.0f) * sy;
